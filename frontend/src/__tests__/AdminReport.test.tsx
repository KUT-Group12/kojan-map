...
describe('AdminReport', () => {
    ...
    it('通報がある場合に正しく表示される', () => {
        render(<AdminReport reports={mockReports} onDeletePost={jest.fn()} onResolveReport={jest.fn()} />);
    ...
    it('通報が未処理→処理済み、新しい順にソートされる', () => {
        render(<AdminReport reports={mockReports} onDeletePost={jest.fn()} onResolveReport={jest.fn()} />);
        const displayedReasons = screen.getAllByText(/.+/).filter((el) => ['不適切な投稿', 'スパム投稿', '迷惑行為'].includes(el.textContent || '')).map(el => el.textContent);
        // 未処理（最新）→未処理（古い）→処理済み
